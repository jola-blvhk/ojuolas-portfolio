...
const ThemeToggle: React.FC = () => {
  ...
  return (
    <div
      className={`relative grid content-center justify-center ${isActive ? 'bg-p 0' : ''} h-15 w-15 cursor-pointer rounded-full p-1`}
      onClick={toggleTheme}
    >
      <button className={switchClasses}>
        {isActive ? (
          ...
          <TbSunFilled color={'#ffffff'} className="text-white" size={45} />
        )}
      </button>
    </div>
  );
