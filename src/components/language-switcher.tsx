...
export function LanguageSwitcher() {
  ...
  const activeLocale = (supportedLocales as readonly string[]).includes(
    pathSegments[0] as any,
  )
      ...
      {supportedLocales.map((loc, idx) => (
        <React.Fragment key={loc}>
          <Link
            ...
            className={`md:px-10px flex items-center gap-1 px-[12px] py-4 ${
              loc === activeLocale
                ? 'text-brand-shadow-highlight font-medium'
                : 'text-brand-shadow'
            }`}
            prefetch={true}
          >
          ...
          {idx < supportedLocales.length - 1 ? (
            <span
              aria-hidden
              className="bg-brand-grey mx-[7px] h-[19px] w-px md:mx-[14px]"
            />
          ) : null}
        </React.Fragment>
      ))}
